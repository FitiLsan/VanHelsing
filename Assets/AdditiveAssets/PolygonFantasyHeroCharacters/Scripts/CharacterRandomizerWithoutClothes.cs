...
namespace PsychoticLab2
{
    public enum Gender { Male, Female }
    public enum SkinColor { White, Brown, Black, Elf }
    public enum FacialHair { Yes, No }

    public class CharacterRandomizerWithoutClothes : MonoBehaviour
    {
        [Header("Demo Settings")]
        public bool repeatOnPlay = false;
        public float shuffleSpeed = 2.0f;
        ...
        void Randomize()
        {
            ...
            // select skin 33% chance for each
            ...
            switch (gender)
            {
                case Gender.Male:
                    ...
                    RandomizeByVariable(male, gender, facialHair, skinColor, isHairless);
                    break;
                ...
                case Gender.Female:
                    ...
                    // initialize randomization
                    RandomizeByVariable(female, gender, facialHair, skinColor, isHairless);
                    break;
            }
        }

        // randomization method based on previously selected variables
        void RandomizeByVariable(CharacterObjectGroups cog, Gender gender, FacialHair facialHair, SkinColor skinColor, bool isHairless)
        {
            if (allGender.all_Hair.Count != 0 && !isHairless)
            ...
            if (cog.facialHair.Count != 0 && facialHair == FacialHair.Yes && gender == Gender.Male)
                ActivateItem(cog.facialHair[Random.Range(0, cog.facialHair.Count)]);

            if (cog.torso.Count != 0)
                ActivateItem(cog.torso[0]);

            if (cog.arm_Upper_Right.Count != 0)
                ActivateItem(cog.arm_Upper_Right[0]);

            if (cog.arm_Upper_Left.Count != 0)
                ActivateItem(cog.arm_Upper_Left[0]);

            if (cog.arm_Lower_Right.Count != 0)
                ActivateItem(cog.arm_Lower_Right[0]);

            if (cog.arm_Lower_Left.Count != 0)
                ActivateItem(cog.arm_Lower_Left[0]);

            if (cog.hand_Right.Count != 0)
                ActivateItem(cog.hand_Right[0]);

            if (cog.hand_Left.Count != 0)
                ActivateItem(cog.hand_Left[0]);

            if (cog.hips.Count != 0)
                ActivateItem(cog.hips[0]);

            if (cog.leg_Right.Count != 0)
                ActivateItem(cog.leg_Right[0]);

            if (cog.leg_Left.Count != 0)
                ActivateItem(cog.leg_Left[0]);
